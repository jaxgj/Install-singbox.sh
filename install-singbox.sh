...
# 安装证书到统一目录
# 兼容逻辑：服务运行则热重载，未运行则静默跳过，不报错、不中断脚本
...
echo "密码：${SS_PASS}"
SS_LINK="ss://$(printf "%s" "aes-256-gcm:${SS_PASS}" | base64 -w0)@${DOMAIN}:${SS_PORT}#SS-${DOMAIN}"
...
echo "密码：${HY2_PASS}"
echo "传输协议：QUIC/h3"
HY2_LINK="hysteria2://${HY2_PASS}@${DOMAIN}:${HY2_PORT}/?sni=${DOMAIN}&alpn=h3#HY2-${DOMAIN}"
